...
const NAV = [
  { name: 'Dashboard',        path: '/',      icon: LayoutDashboard, sub: 'Operations & catalog' },
  { name: 'Add New Product',  path: '/apply', icon: FilePlus,        sub: 'New product application' },
];
...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex text-slate-900 dark:text-slate-200 font-sans relative overflow-hidden bg-gradient-to-br from-[#f4f7fb] via-[#ffffff] to-[#fff0f3] dark:from-[#080b14] dark:via-[#090e1c] dark:to-[#170a11] transition-colors duration-500">
      ...
      <aside className="hidden md:flex flex-col w-64 fixed inset-y-0 left-0 z-30
        ...
        <div className="flex items-center gap-3 px-5 h-[68px] border-b border-slate-200/60 dark:border-white/[0.05] shrink-0">
          ...
          <div>
            <h1 className="font-display font-bold text-base text-slate-900 dark:text-white leading-none tracking-tight">Vectorflow</h1>
            <span className="text-[10px] font-semibold tracking-[0.12em] text-brand-500 uppercase block mt-0.5">Catalog Portal</span>
          </div>
        </div>
        ...
        <div className="p-3 border-t border-slate-200/60 dark:border-white/[0.05] space-y-2 shrink-0">
          ...
          <div className="px-3.5 py-3 rounded-xl bg-emerald-500/5 dark:bg-emerald-500/8 border border-emerald-500/15">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[10px] font-semibold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider">Live Connection</span>
            </div>
            <p className="text-xs font-medium text-slate-600 dark:text-dark-300">Neon PostgreSQL Active</p>
          </div>
        ...
        <div className="px-5 pb-4 pt-1">
          <p className="text-[10px] font-mono text-slate-350/50 dark:text-dark-700">v1.0.0 · Vectorflow Portal</p>
        </div>
      </aside>
      ...
      <div className="md:hidden fixed top-0 inset-x-0 h-14 flex items-center justify-between px-4 z-40
        ...
        <div className="flex items-center gap-2.5">
          ...
          <span className="font-display font-bold text-base text-slate-900 dark:text-white">Vectorflow</span>
        </div>
            ...
            <motion.nav key="drawer"
              ...
              <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200/60 dark:border-white/[0.05]">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <p className="font-display font-bold text-slate-900 dark:text-white text-base leading-none">Vectorflow</p>
                    <p className="text-[10px] text-brand-500 font-semibold tracking-wider uppercase mt-0.5">Catalog Portal</p>
                  </div>
                </div>
                <button onClick={() => setMobileOpen(false)}
                  className="p-1.5 rounded-lg text-slate-450 hover:bg-slate-100 dark:hover:bg-dark-800 transition-colors">
                  <X size={17} />
                </button>
              </div>
                    ...
                    <Link key={path} href={path}
                      className={`flex items-center gap-3.5 px-4 py-3.5 rounded-xl font-semibold text-sm transition-all duration-200 relative ${
                        active
                          ...
                          : 'text-slate-500 dark:text-dark-400 hover:text-slate-800 dark:hover:text-dark-100 hover:bg-slate-100/50 dark:hover:bg-dark-900/50'
                      }`}>
                      {active && (
